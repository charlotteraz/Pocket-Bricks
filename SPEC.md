...
`src/data/sets/*.json` hold three fixed sets (phase 5, later widened from the plan's original "one fixed set" scope to let users pick between builds): a striped **lighthouse** with an attached keeper's cottage (55 bricks), a **car** (22 bricks), and a **flower** in a pot (22 bricks). All three were reworked to use the slope/wedge shapes (see "Shape registry" above) once those existed, rather than staying box-only:

- **Lighthouse**: the tower's flat roof cap became a proper two-sided gable using 4x `slope2x2` (2 at `rotation:0`, 2 at `rotation:180`, forming a ridge along X) — same piece count as the flat version it replaced, just a real pitched roof instead of a cap. Also added: two window cutouts (a flat brick2x4 split into a `brick2x2` + a blue 1x1 window + filler pieces, at two different stripe heights), a rooftop antenna, a cottage chimney (3 stacked black 1x1s), and a lamppost by the door.
- **Car**: a `slope3x2` at `rotation:90` forms a sloped hood ramping from the chassis up to windshield height, and a second `slope3x2` at `rotation:270` tapers the roofline down over the rear for a hatchback profile — both fit exactly because `slope3x2`'s effective footprint at 90°/270° is `[2,3]`, matching the car's 2-wide/3-deep body (getting that orientation right is why this shape was added instead of reusing `slope2x2`). Also added: grille-style headlights beside the front bumper, a two-tone rear window, and a small spoiler nub sitting on the tapered roof's rear cell.
- **Flower**: the old plus-shaped cluster of 9 flat 1x1 petals became a 2x2 `wedge2x2` hub with 4 pointed petals (one per side, `rotation` 0/90/180/270) genuinely pointing outward, plus alternating leaf orientations up the stem (X-facing, then Z-facing, then X-facing again) so the foliage reads as layered rather than one flat block from every angle, and a small second bud on its own short stem.
